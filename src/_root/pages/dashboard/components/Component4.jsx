...
const StyledClockCircleOutlined = styled(HistoryOutlined)`
...
function Component4(props) {
  ...
  const yilSecimiKmBasinaMaliyetler = watch("yilSecimiKmBasinaMaliyetler");

  useEffect(() => {
    const yilSecimiValue = watch("yilSecimiKmBasinaMaliyetler");
    if (!yilSecimiValue) {
  ...
  useEffect(() => {
    if (isModalVisible === true) {
      setValue("yilSecimiKmBasinaMaliyetler", null);
      // reset({
        ...
        <Modal title="Tarih Seçimi" centered open={isModalVisible} onOk={handleOk} onCancel={handleCancel} destroyOnClose>
          {modalContent === "Yıl Seç" && (
            ...
            >
              <div>Yıl Seç:</div>
              <Controller
                name="yilSecimiKmBasinaMaliyetler"
                control={control}
