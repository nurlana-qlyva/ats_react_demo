...
import {
  CikisFisleriListGetService,
  CikisFisleriListSearchService,
} from "../../../api/services/cıkısfıs_services";
...
const CikisFisleri = () => {
  ...
  useEffect(() => {
    setLoading(true);
    CikisFisleriListGetService(tableParams?.pagination.current).then((res) => {
  ...
  useEffect(() => {
    if (search.length >= 3) {
      ...
      CikisFisleriListGetService(tableParams?.pagination.current).then(
        (res) => {
          setData(res?.data.materialList);
          setTableParams({
            ...tableParams,
